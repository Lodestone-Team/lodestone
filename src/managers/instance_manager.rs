...
impl InstanceManager {
    pub fn new(path: PathBuf) -> Result<InstanceManager, String> {
        ...
        let mut tunnel_manager = TunnelManager::new(
            path.join(".lodestone").join("bin").join("frpc"),
        ...
        for entry in fs::read_dir(path_to_instances.as_path()).unwrap() {
            ...
            if entry.file_type().unwrap().is_dir() {
                if entry.path().join(".lodestone_config").exists() {
                    ...
                    instance_list.push(server_instance.instance_config.clone());
                    instance_collection
    ...
    pub async fn create_instance(
        ...
        let mut properties_manager =
            PropertiesManager::new(path_to_instance.join("server.properties")).unwrap();
        ...
        self.instance_list.push(instance.instance_config.clone());
